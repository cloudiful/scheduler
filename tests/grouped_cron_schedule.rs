...
#[tokio::test]
async fn grouped_cron_replay_all_runs_each_missed_anchor_slot_in_order() {
    ...
    let first_due = now_floor - TimeDelta::minutes(2) + TimeDelta::seconds(20);
    let second_due = now_floor - TimeDelta::minutes(1) + TimeDelta::seconds(20);
    let expected = vec![first_due, second_due, now_floor + TimeDelta::seconds(20)];

    store
        .save(&JobState::new("grouped-cron-replay-all", Some(first_due)))
        .await
        .unwrap();

    let job = Job::without_deps(
        "grouped-cron-replay-all",
...
#[tokio::test]
async fn grouped_cron_catch_up_once_replays_only_the_latest_due_slot() {
    let store = Arc::new(InMemoryStateStore::new());
    let scheduler = Scheduler::new(SchedulerConfig::default(), store.clone());
    ...
    store
        .save(&JobState::new("grouped-cron-catch-up-once", Some(first_due)))
        .await
        .unwrap();

    let job = Job::without_deps(
        "grouped-cron-catch-up-once",
        Schedule::grouped_cron(
            CronSchedule::parse("* * * * *").unwrap(),
            Duration::from_secs(40),
            4,
    ...
    assert_eq!(report.history[0].scheduled_at, now_floor + TimeDelta::seconds(20));
...
#[tokio::test]
async fn grouped_cron_skip_advances_state_then_waits_for_the_next_future_slot() {
    let store = Arc::new(InMemoryStateStore::new());
    let scheduler = Scheduler::new(SchedulerConfig::default(), store.clone());
    let handle = scheduler.handle();
    let now_floor = minute_floor(Utc::now());
    let first_due = now_floor - TimeDelta::minutes(2) + TimeDelta::seconds(20);
    let expected_next = now_floor + TimeDelta::minutes(1) + TimeDelta::seconds(20);

    store
        .save(&JobState::new("grouped-cron-skip", Some(first_due)))
        .await
        .unwrap();
...
async fn grouped_cron_overlap_allow_parallel_runs_due_replay_concurrently() {
    let store = Arc::new(InMemoryStateStore::new());
    let scheduler = Scheduler::new(SchedulerConfig::default(), store.clone());
    ...
    let now_floor = minute_floor(Utc::now());
    let first_due = now_floor - TimeDelta::minutes(3) + TimeDelta::seconds(20);

    store
        .save(&JobState::new("grouped-cron-parallel", Some(first_due)))
        .await
        .unwrap();

    let job = Job::without_deps(
        "grouped-cron-parallel",
        Schedule::grouped_cron(
            CronSchedule::parse("* * * * *").unwrap(),
            Duration::from_secs(40),
            4,
...
async fn grouped_cron_state_is_restored_across_scheduler_instances() {
    let store = Arc::new(InMemoryStateStore::new());
    ...
    let seen = invocations.clone();
    let now_floor = minute_floor(Utc::now());
    let first_due = now_floor - TimeDelta::minutes(1) + TimeDelta::seconds(20);

    store
        .save(&JobState::new("grouped-cron-restore", Some(first_due)))
        .await
        .unwrap();

    let job = Job::without_deps(
        "grouped-cron-restore",
        Schedule::grouped_cron(
            CronSchedule::parse("* * * * *").unwrap(),
            Duration::from_secs(40),
            4,
    ...
    let saved_state = store.load("grouped-cron-restore").await.unwrap().unwrap();
    assert_eq!(saved_state.next_run_at, Some(now_floor + TimeDelta::seconds(20)));
